...
Usage:
    python scripts/prepare_test_data.py --source "C:/path/to/unsplash/photos" --count 100
"""
...
def copy_random_images(source_files, dest_dir, count):
    ...
    if len(source_files) <= count:
        ...
        print(f"[WARN] Source has only {len(source_files)} images, copying all")
    else:
        selected = random.sample(source_files, count)
        print(f"[OK] Selected {count} random images")
    ...
    for i, src_file in enumerate(tqdm(selected, desc="Copying"), 1):
        ...
        try:
            ...
            print(f"[FAIL] Failed to copy {src_file.name}: {e}")

    print(f"[OK] Copied {len(copied)} images successfully")
...
def main():
    ...
    try:
        ...
        if len(image_files) == 0:
            print("[FAIL] No images found in source directory!")
            return
        ...
        print("\n" + "=" * 60)
        print("[OK] TEST DATA PREPARATION COMPLETE")
        print("=" * 60)
