"""
Test SigLIP on Single Image

Verifies SigLIP plugin works correctly.

Phase 1 Day 2 - Image Analysis System v0.1

Usage:
    python test_siglip.py <image_path>
"""
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python test_siglip.py <image_path>")
        sys.exit(1)
    ...
    print("\n" + "=" * 70)
    print("🧪 SIGLIP SINGLE IMAGE TEST")
    print("=" * 70)
    
    # Initialize config for SigLIP
    print("\n📊 Step 1: Loading SigLIP model...")
    start_time = time.time()
    
    config = Config(model_name='siglip')
    model = SigLIP(config.model_path, config.model_name)
    
    load_time = time.time() - start_time
    
    # Show model info
    info = model.get_model_info()
    print(f"✅ SigLIP model loaded")
